...
> **隐私口径（8.8.16 起）**：推送正文含三类信息——① 数字计数（正常/异常/留人工、各红线类别例数）；② **留人工标本的检验号（标本号）与接收时间**（即工作台卡片显示的时间，用于手机端定位标本，已确认不属于病人隐私）；③ **异常项目**（检验项目名 + 数值 + 参考范围 + 方向标记 🔺升高/🔻降低，不显示单位）。**绝不**包含姓名、住院号、床号、科室、ReportDR 等身份信息；摘要按标本展开、最多 8 行，每个标本最多列 6 个异常项（超出标 `等N项`）。
...
- **普通提醒 `active`**：本轮标本**全部能正常通过自动审核**（无红线留人工）→ 标题 `🤖 自动审核完成一轮`（锁屏+横幅）。
...
| 推送正文会出现哪些信息 | 数字计数 + 留人工标本的检验号与接收时间 + 异常项目（项目名/数值/参考范围/方向标记 🔺/🔻，不显示单位）。姓名、住院号、床号、科室等身份信息**永远不会**出现在推送里 |
