...
| 事件 | 标题 | 正文 | 级别 |
|---|---|---|---|
| 自动审核一轮完成 | `🤖 自动审核完成一轮` | `正常 a · 异常 b · 留人工 c`；有异常项时追加按标本展开的摘要——留人工与**通过自动审核的异常标本**同样列出（每标本一行 `标本号 时间` + 一行异常项，如 `钾(K) 6.8 🔺 (3.50-5.30)`） | `active`（普通提醒） |
...
> **隐私口径（8.8.17 起）**：推送正文含三类信息——① 数字计数（正常/异常/留人工、各红线类别例数）；② **标本的检验号（标本号）与接收时间**（即工作台卡片显示的时间，用于手机端定位标本，已确认不属于病人隐私）；③ **异常项目**（检验项目名 + 数值 + 方向标记 + 参考范围，标记放数值与参考范围中间：🔺升高(红)/🔽降低(蓝)/危急/0值，不显示单位）。**绝不**包含姓名、住院号、床号、科室、ReportDR 等身份信息；留人工与通过自动审核的异常标本同样列出异常值（呈现一致，仅推送级别不同）；摘要按标本展开、最多 8 行，每个标本最多列 6 个异常项（超出标 `等N项`）。
