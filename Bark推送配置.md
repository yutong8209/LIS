...
| 事件 | 标题 | 正文 | 级别 |
|---|---|---|---|
| 自动审核一轮完成 | `🤖 自动审核完成一轮` | `正常 a · 异常 b · 留人工 c`；有异常项时追加按标本展开的摘要——留人工与**通过自动审核的异常标本**同样列出（每标本一行 `流水号 时间` + 一行异常项（8.8.22 起标本头用流水号，无流水号回退检验号，如 `流水号 123 14:30`），如 `🚨 钾(K) 6.8 🔺 危急 (3.50-5.30)`；普通异常如 `钾(K) 5.1 🔺 (3.50-5.30)`） | `active`（普通提醒） |
