...
| 事件 | 标题 | 正文 | 级别 |
|---|---|---|---|
| 自动审核一轮完成 | `🤖 自动审核完成一轮` | `正常 a · 异常 b · 留人工 c`；有留人工时追加按标本展开的摘要（每标本一行 `标本号 时间` + 一行异常项，如 `钾(K) 6.8 mmol/L 危急(3.50-5.30)`） | `active` |
| 危急红线留人工 | `⚠️ 自动审核：危急红线 N 例留人工` | 计数 + 红线类别分布（危急值/含负值/传染病阳性/心肌标志物/疑似堵孔各 N 例）+ 按标本展开的异常摘要（危急项附参考范围） | `critical`（穿透勿扰/静音） |
| 审核时长到点自动停止 | `⏰ 自动审核已停止` | 状态说明 | `active` |
| 手动关闭自动审核 | `🛑 自动审核已关闭` | 状态说明 | `active` |

> **隐私口径（8.8.14 起）**：推送正文含三类信息——① 数字计数（正常/异常/留人工、各红线类别例数）；② **留人工标本的检验号（标本号）与接收时间**（即工作台卡片显示的时间，用于手机端定位标本，已确认不属于病人隐私）；③ **异常项目**（检验项目名 + 数值 + 单位 + 方向标记，危急项附参考范围）。**绝不**包含姓名、住院号、床号、科室、ReportDR 等身份信息；摘要按标本展开、最多 8 行，每个标本最多列 6 个异常项（超出标 `等N项`）。
...
| 推送正文会出现哪些信息 | 数字计数 + 留人工标本的检验号与接收时间 + 异常项目（项目名/数值/单位/方向，危急项附参考范围）。姓名、住院号、床号、科室等身份信息**永远不会**出现在推送里 |
