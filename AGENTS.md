...
- **门禁隔离与不完整标本自动化绝缘（8.18.0）**：
  ...
  - 回归：`node audit_completeness_guard_test.mjs`（89 断言，全部通过）。
- **补审轮三态返回值（8.18.1，修 8.18.0 引入的漏审洞）**：`clickNativeAuditButton` / `executeNativeAudit` 的返回值是**三态**——`true`=成功 / `false`=失败 / `'incomplete'`=被原生「必填项目未存数据」弹窗拦下。**字符串是 truthy**，8.18.0 在 `auditOneQueueItemOnce` 结尾写成 `return { ok: !!result }` → `!!'incomplete'` = true → 调用方 `successCount++` + `queue.done.push` + `markSpecimenAuditedInMem`（**工作台把标本隐藏**）+ 日志记「正常」，而原生审核其实被取消了 → **漏审且现场看不见**。现为 `ok: result === true`。⚠️ 检查任何 `if (result)` / `if (r.ok)` 时都要问：`'incomplete'` 落到哪一支？
- **漏结果审核实时监测（8.18.1）**：用 **LIS 自己的完整度口径**反查漏审——`IsComplete` 是原生在 ReportSave 时算好写库的（`cache/js/iMedicalLIS/lis/js/jsLisReportResultM.js:1434`），所以「工作列表里 `IsComplete='0'`（结果空）或 `'2'`（不完整）」**且**「`Status='3'`（已审核）」= **审核时结果就不完整却被放行**。
  - 查询：复用 `QryWorkList`（与原生 `ShowWorkList` 同一个接口），`P0=''`（**全部状态**，含已审核——这是能抓到的关键）+ `P5='^<type>'`（完整度过滤，原生 `SearchByResultType` 同款，见同文件 4873 行）。⚠️ **不调原生 `ShowWorkList`**——那会刷掉用户的原生列表。
  - 本地必须**再按 `IsComplete` 复核一遍**（服务端若忽略 `P5`，这里兜住）；只认 `Status='3'`，**不认 `'4'`**（复查=待复审，不是已审核）。
