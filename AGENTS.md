...
- **「能不能审」的完整度判定必须走 `specimenCompleteness(row, cached)`（8.17.9）**：它返回 `{ok, missing, reason}`，内部复用 `isSpecimenActuallyComplete`（单一事实来源）。LIS 字段口径（用 `cache/` 里真实 `QryWorkList` 响应核过）：`IsComplete='1'`=完整（`NoResRows` 空）、`'2'`=部分结果（`NoResRows`=**还缺几项**）、`'0'`=无结果（`NoResRows`=**该组合项目总项数**）。
  - **三道自动闸门一个都不能少**：① F4 主循环逐条复核（`_cmpEarly`）；② 「审核前最终闸门」（拿到最新分类之后、`clickNativeAuditButton` 之前，按 `liveRow || findWSSpecimenByReportDR || queueItemNativeRow` 判）；③ 补审轮 `auditOneQueueItemOnce`。**补审的语义是「救回未确认的」，不是「绕过完整度」**——8.17.9 之前补审轮没有闸门，主循环拦下的标本会在补审轮被原样放行。
  - ⚠️ **一律 fail-closed**：拿不到行（跨整页刷新续跑 / 切组 / 批审中途工作台刷新把行挤掉）**也绝不放行**——走 `requeueAuditItem(..., {tech:true})` → 补审 → 留人工。旧写法 `if (liveRow && !isSpecimenActuallyComplete(liveRow))` 在 `liveRow` 为空时**整条判定被静默跳过**，那是漏结果审核最容易钻的洞。
  - 详情面板「审核」按钮**只提示、不硬拦**（`showToast` + `dbg`）：从详情手动点审核是「确需人工审核」的有意通道（见 HANDTEST §22），硬拦会把它锁死。
  - **审核后自检**（`verifyAuditedCompleteness(queue)`，在 `continueAuditQueue` 收尾调用）：用**最新工作列表**（`loadWL`）复核本批 `queue.done` 的标本，发现不完整立刻红色 toast + `LIS危急告警` 推送 + 状态时间线。**读不到列表 / 标本已不在列表 → 不下结论**（宁可漏报也不误报）。
  - **审核留痕 `LIS_AuditTrace`（8.17.10）**：三条自动路径（人工批审 F4 / 补审轮 / 详情面板·回车）每次审掉标本都记一条「**审核那一刻**的完整度快照（`IsComplete` / `NoResRows`）+ 走的哪条路径」。起因：8.17.9 那条现场标本（检验号 26091800246）结果补全后就再也看不出当时状态，只能靠回忆。有了它，下次同类问题能**直接定性**：审核时 `ic='1'`（LIS 说完整）而事后不完整 → **LIS 侧判错了**（脚本无信号可查，需要「应有项数」对照表）；审核时 `ic='2'/'0'` 或 verdict 非空 → **闸门被绕过 / 用了过期数据**（脚本侧问题）。现场诊断：F12 控制台 `lisAuditTrace()`（全部）或 `lisAuditTrace('<检验号>')`。环形 800 条。
  - **「应有项数」观察模式（8.17.11）**：从 LIS 自己 `IsComplete='0'` 行的 `NoResRows`（= 该组合项目的**项目总数**）学出 `组合项目 → 应有项数`（取历史最大值 + 记观测次数，存 `LIS_ExpectedItems`，在 `applyResults` 里学）。审核留痕里带 `exp/act/short/seen`，现场入口 `lisExpectedItems()`。
    ⚠️ **只记不拦**：`specimenCompleteness` / 各闸门**一律不许**依赖它——「减项开单」的组合项目会误拦。等留痕里攒够数据（能看到它抓不抓得到真问题、会不会误报）再决定是否升级成硬拦。回归里有「闸门不依赖 expectedItem」的断言钉住。
  - 回归：`node audit_completeness_guard_test.mjs`（100 断言；8.17.10 应 5 项失败 / 8.17.9 应 11 / 8.17.8 应 24）。
